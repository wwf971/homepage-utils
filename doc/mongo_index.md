...
An elasticsearch-based index system for mongodb.
...
- `POST /mongo-index/create` Create index
  - Body: `{name, esIndex, collections: [{database, collection}]}`
- `GET /mongo-index/{indexName}` Get single index
- `PUT /mongo-index/{indexName}/update` - Update index collections
  - Body: `{esIndex, collections: [{database, collection}]}`
...
- `POST /mongo-index/{indexName}/search` - Search in ES
  - Body: `{query, search_in_paths, search_in_values}`

### Document CRUD
- `PUT /mongo-index/{indexName}/doc/{dbName}/{collName}/{docId}/update` Update doc with indexing
  - Body: `{updateDict: {field: value}, updateIndex: true}`
...
An auxiliary collection in each mondodb database. Schema of `__IndexQueue__`:
```
{
  docId: String,
  collection: String,
