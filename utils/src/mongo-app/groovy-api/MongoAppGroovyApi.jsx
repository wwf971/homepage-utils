...
import { fileStore, initFileStore } from '@wwf971/homepage-utils-utils';
...
const MongoAppGroovyApi = observer(({ store }) => {
  ...
  const handleAddFolder = async ({ fileAccessPointId, path }) => {
    ...
    const result = await groovyApiStore.addFolder(backendUrl, appId, fileAccessPointId, path || '');
  ...
  const handleUpdateFolder = async (oldFolder, nextFolder, onSuccess) => {
    if (!oldFolder?.fileAccessPointId || oldFolder.path == null) {
      setError('Original folder reference is required');
      return;
    }
    if (!nextFolder?.fileAccessPointId) {
      setError('File access point is required');
      return;
    }
    ...
    const result = await groovyApiStore.updateFolder(
      backendUrl,
      appId,
      oldFolder.fileAccessPointId,
      oldFolder.path || '',
      nextFolder.fileAccessPointId,
      nextFolder.path || ''
    );
    ...
    if (result.code === 0) {
      ...
      setError(result.message || 'Failed to update folder');
    }
  };
