...
7. Check frontend code utilizing these backend apis if they need to update their useage of updated backend apis. In case api url/parameters do not change, it's likely that they don't need to change.
...
10. Update docs including but not limited to:
   ...
   - `obj_version.md` (if versioning/status/history semantics change)
...
- All columns related to time should be timestamp based in principle, and each timestamp column should be paired with a timezone column. The naming style of such pair should be `...At` with nullable `...AtTz`, such as `updatedAt` and `updatedAtTz`.
   - Timestamp-based comlumns should at least have microsecond precision. In postgreSQL, `timestamptz` would be sufficient. 
